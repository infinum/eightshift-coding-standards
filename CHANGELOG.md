...
## [3.1.1](https://github.com/infinum/eightshift-coding-standards/compare/3.1.0...3.1.1)

### Changed

- Updated `phpunit/phpunit` to `^12.5.22` to address security advisory GHSA-qrr6-mg7r-m243 (argument injection via newline in PHP INI values).
- Updated CI matrix: dropped PHP 7.4, 8.0, and 8.1; minimum tested version is now PHP 8.2.
- Migrated PHPUnit test runner from `AllSniffs.php` to direct directory discovery for PHPUnit 10+ compatibility.

### Fixed

- `HelpersEscapeSniff`: Fixed false positives for fully-qualified class name calls like `ProjectVendor\EightshiftLibs\Helpers\Helpers::render()` — the sniff was matching the first `Helpers` string (a namespace segment, not the class name) and incorrectly treating the call as unescaped output.
- `HelpersEscapeSniff`: Fixed partial namespace import check being too permissive — `use Vendor\EightshiftLibs\Helpers; echo Helpers::render()` now correctly reports an error since `Helpers` is a namespace alias, not the class itself.
